...
class LiveKitPlugin : FlutterPlugin, MethodCallHandler {
  ...
  override fun onAttachedToEngine(@NonNull flutterPluginBinding: FlutterPlugin.FlutterPluginBinding) {
    Trace.beginSection("LK::Plugin::onAttachedToEngine")
    try {
      Log.d(TAG, "LiveKitPlugin::onAttachedToEngine thread=${Thread.currentThread().name}")
  ...
  private fun handleStartVisualizer(@NonNull call: MethodCall, @NonNull result: Result) {
    Trace.beginSection("LK::handleStartVisualizer")
    try {
      val trackId = call.argument<String>("trackId")
      val visualizerId = call.argument<String>("visualizerId")
      Log.d(TAG, "handleStartVisualizer thread=${Thread.currentThread().name} trackId=$trackId visualizerId=$visualizerId")
  ...
  private fun handleStartAudioRenderer(@NonNull call: MethodCall, @NonNull result: Result) {
    Trace.beginSection("LK::handleStartAudioRenderer")
    try {
      val trackId = call.argument<String>("trackId")
      val rendererId = call.argument<String>("rendererId")
      val formatMap = call.argument<Map<String, Any?>>("format")
      Log.d(TAG, "handleStartAudioRenderer thread=${Thread.currentThread().name} trackId=$trackId rendererId=$rendererId")
  ...
  override fun onMethodCall(@NonNull call: MethodCall, @NonNull result: Result) {
    Trace.beginSection("LK::onMethodCall::${call.method}")
    try {
      Log.d(TAG, "onMethodCall thread=${Thread.currentThread().name} method=${call.method}")
      when (call.method) {
        "startVisualizer" -> {
  ...
  override fun onDetachedFromEngine(@NonNull binding: FlutterPlugin.FlutterPluginBinding) {
    Trace.beginSection("LK::Plugin::onDetachedFromEngine")
    try {
      Log.d(TAG, "LiveKitPlugin::onDetachedFromEngine thread=${Thread.currentThread().name}")
